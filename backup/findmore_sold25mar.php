		...
		if ($mySql->getRecordsCount()> 0)
		{
			...
			foreach($Properties_array as $property)
			{
				...
				if($mySql->getRecordsCount())
				{
					...
					$mySql->query("SELECT s.Polygon_id, s.Sub_id, s.Subdivision, s.Count FROM Real_Listing r, Real_Polygon p, Polygon_Subdivision s where r.Location_id = p.Location_id and p.Polygon_id = s.Polygon_id and s.Sub_id > 0 and r.Property_id = ". $Property_id." order by s.Count desc  LIMIT 1");
					
					if($mySql->getRecordsCount())
					{
						$Base_Location_id= $mySql->fetchArray(MYSQL_ASSOC);
						//echo "<pre>"; print_r($Base_Location_id); 
						trace("Insert data in to MoreLike_Listings_Sold table");
						//echo "INSERT INTO MoreLike_Listings_Sold (Property_id, qProp_id, MLS, Polygon_id, ListPrice, SquareFeet, PricePerFoot, Bedrooms, Bathrooms, Latitude, Longitude, geodna)(SELECT ".$Property_id.", r.Property_id, r.ListingNumber AS MLS , p.Polygon_id,  r.ClosePrice, r.SquareFeet, ROUND(r.ClosePrice/r.SquareFeet,0), r.TotalBedrooms as Bedrooms, r.TotalBathrooms as Bathrooms, l.Latitude, l.Longitude, l.geodna  from tb_Subdivision_City t, Polygon_Subdivision s, Real_Polygon p, Real_Location l, Real_Listing r where t.Sub_id = s.Sub_id AND s.Polygon_id = p.Polygon_id AND p.Location_id = r.Location_id AND r.Location_id = l.Location_id AND r.ClosePrice BETWEEN ".$MinPrice ." AND ".$MaxPrice." AND r.CloseDate >'". $sold_modified."' AND r.Status = 'S' AND t.Common_id = ".$Base_Location_id['Sub_id'].")";echo "<br/>";echo "<br/>";

						$mySql->query("INSERT INTO MoreLike_Listings_Sold (Property_id, qProp_id, MLS, Polygon_id, ListPrice, SquareFeet, PricePerFoot, Bedrooms, Bathrooms, Latitude, Longitude, geodna)(SELECT ".$Property_id.", r.Property_id, r.ListingNumber AS MLS , p.Polygon_id,  r.ClosePrice, r.SquareFeet, ROUND(r.ClosePrice/r.SquareFeet,0), r.TotalBedrooms as Bedrooms, r.TotalBathrooms as Bathrooms, l.Latitude, l.Longitude, l.geodna  from tb_Subdivision_City t, Polygon_Subdivision s, Real_Polygon p, Real_Location l, Real_Listing r where t.Sub_id = s.Sub_id AND s.Polygon_id = p.Polygon_id AND p.Location_id = r.Location_id AND r.Location_id = l.Location_id AND r.ClosePrice BETWEEN ".$MinPrice ." AND ".$MaxPrice." AND r.CloseDate >'". $sold_modified."' AND r.Status = 'S' AND r.`PropertyType`='".$PropertyType."' AND t.Common_id = ".$Base_Location_id['Sub_id'].")");
					}
				}
		
				/*********************************************Base Property Data************************************************/
				
				trace("fetch Base property data from MoreLike_Listings_Sold for matching...");
				...
				if($mySql->getRecordsCount())
				{
					...
					//echo "<pre>"; print_r($BaseMoreLike); exit;
					...
					do
					{
						$mySql->query("SELECT * FROM MoreLike_Listings_Sold 
					...
					$mySql->query("SELECT Like_id FROM MoreLike_Listings_Sold WHERE Property_id =". $Property_id ." AND  Like_id NOT IN ( SELECT Like_id FROM MoreLike_Listings_Sold WHERE Property_id =". $Property_id ." AND geodna LIKE concat(left('".$Base_Geodna."',".$concat_limit."),'%') )");
					...
					{
						$mySql->query("DELETE FROM MoreLike_Listings_Sold WHERE Property_id=".$Property_id." AND Like_id=".$morelike_result['Like_id'] );
					}
