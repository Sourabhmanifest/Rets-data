<?php
    ...
    try
    {
       ...
       $mySql->connect();
		...
		if($mySql->getRecordsCount())
		{
			...
			$mySql->query('SELECT c.Polygon_id, s.Subdivision, s.Sub_id, SUM( Count ) AS totalcnt FROM Location_Subdivision s, Real_Polygon p, Polygon_Center c WHERE s.Location_id = p.Location_id AND p.Polygon_id = c.Polygon_id GROUP BY c.Polygon_id, s.Subdivision ORDER BY c.Polygon_id, s.Subdivision');
	...
	function revisit_location_subdivision()
	{
		...
		if($mySql->getRecordsCount())
		{
			...
			{
				//echo '<pre>';print_r($tb_Subdivision_City_property);  exit;
				...
				$mySql->query("SELECT * FROM tb_Subdivision_City  WHERE Accepted=0 AND  City_id=".$city_id." AND Subdivision LIKE '%".$subdivision."%'");
				
				if($mySql->getRecordsCount())
				{
					$tb_Subdivision_matched_property=$mySql->fetchArray(MYSQL_ASSOC);
					//echo '<pre>';print_r($tb_Subdivision_matched_property);//exit;
					...
					if($mySql->getRecordsCount())
					{
						...
						//echo '<pre>';print_r($Location_Subdivision_matched_properties); //exit;
						
						foreach($Location_Subdivision_matched_properties as $Location_Subdivision_matched_record)
						{
							$mySql->query("UPDATE  Location_Subdivision SET  Sub_id=".$tb_Subdivision_City_property['Sub_id']." AND  Subdivision='".$tb_Subdivision_City_property['Subdivision']."' WHERE Location_id=".$Location_Subdivision_matched_record['Location_id']." AND Sub_id= ".$Location_Subdivision_matched_record['Sub_id']);
						}
					}
			...
			{	
				...
				if($mySql->getRecordsCount() )
				{
					$mySql->fetchAllRecords($Location_Subdivision_properties, MYSQL_ASSOC);
					//echo '<pre>';print_r($Location_Subdivision_properties); 

					foreach($Location_Subdivision_properties as $Location_Subdivision_property)
					{
