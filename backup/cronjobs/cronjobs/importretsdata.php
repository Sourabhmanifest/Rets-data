                    ...
                    if ($sourceRecordsCount)
                    {
                        foreach ($sourceRecords as $sourceRecord)
                        {
                            ...
                            try
                            {
                                ...
                                $realListing = null;
                 /****************************************Lat along Verify Changes by Neel on16 may 2015***************************************/               /*
								$sourceRecord['FullAddress']="";
                                if($sourceRecord['UnitNumber']=='' && $sourceRecord['StreetBuildingNumber']=='')
								{
											$sourceRecord['FullAddress'] = $sourceRecord['StreetNumber'].' '.$sourceRecord['StreetName'].' '.$sourceRecord['StreetSuffix'].' '.$sourceRecord['StreetDirPrefix'].', '.$sourceRecord['City_Decoded'];
								}
								elseif($sourceRecord['UnitNumber']!='' && $sourceRecord['StreetBuildingNumber']=='')
								{
											$sourceRecord['FullAddress'] = $sourceRecord['UnitNumber'].' - '.$sourceRecord['StreetNumber'].' '.$sourceRecord['StreetName'].' '.$sourceRecord['StreetSuffix'].' '.$sourceRecord['StreetDirPrefix'].', '.$sourceRecord['City_Decoded'];
								}
								elseif($sourceRecord['UnitNumber']!='' && $sourceRecord['StreetBuildingNumber']!='')
								{
											$sourceRecord['FullAddress'] = $sourceRecord['StreetBuildingNumber'].' Bldg '.$sourceRecord['UnitNumber'].' - '.$sourceRecord['StreetNumber'].' '.$sourceRecord['StreetName'].' '.$sourceRecord['StreetSuffix'].' '.$sourceRecord['StreetDirPrefix'].', '.$sourceRecord['City_Decoded'];
								}*/
							//	echo "in";exit;
								//$lat_long=get_lat_long($sourceRecord['FullAddress']);
								//echo "<pre>";print_r($lat_long);exit;
								//$ex_lat_long=explode(",",$lat_long);
								//$sourceRecord['Latitude'] =$ex_lat_long[0];
								//$sourceRecord['Longitude'] =$ex_lat_long[1];
				/************************************************************************************************/
